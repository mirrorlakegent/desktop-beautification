...
## v21（❌ FrostOpacity 极低时毛玻璃变白，三条全不通过）
- **现象**：FrostOpacity=8 → 全白；=162 → 深灰无模糊；较高值 → 同一 _frostBmp 内不同盒子不一致
  （部分正确显示壁纸模糊、部分白）。
- **v21 尝试**：`Math.Max(FrostOpacity, 20)` 最小染色 + `_frostBmp` null fallback。
- **结果**：未解决——三条全不通过。

## v22（根因修复：CopyFromScreen 自捕获反馈循环，待真机复验）

### 决定性证据
图3 的**同一 _frostBmp 内部不一致性**（"临时""工作"正确显示星云模糊 vs "未分类""工具"纯白）
证明问题在**捕获内容**而非渲染路径——如果渲染有问题，四个盒子应该一致。

### 真正的根因
`EnsureFrostCapture:2321` 的 `CopyFromScreen()` 截取的是**屏幕合成输出**（包含我们自己 layered window
的上一帧），而非窗口背后的纯壁纸。这创建了**自捕获反馈循环**：

| FrostOpacity | 上一帧外观 | CopyFromScreen 捕获到 | 本帧渲染 |
|---|---|---|---|
| 极低 (0~20) | 亮色/白色（淡染色+亮截图） | 亮的模糊内容 | 更白 → 死锁 |
| 高 (150~200) | 暗色（重染色覆盖一切） | 暗的模糊内容 | 更暗 → 看不见模糊 |

v18"正常"是因为测试用中等值（暗色掩盖了自捕获痕迹）且只随手开关未测极端值。

### v22 修复
- **`EnsureFrostCapture`**：`CopyFromScreen` 前先 `ShowWindow(SW_HIDE)` 隐藏窗口 + `Sleep(50)` 让 DWM
  处理 → 截取真壁纸 → `ShowWindow(SW_SHOW)` 恢复。仅 frost session 首次/失效时执行一次。
- **回退 v21**：删除 `Math.Max(FrostOpacity, 20)`，恢复 v18 原始直传（渲染路径本身没问题）。
- 已发布 ds2（exe Aug 26 06:21），0 错误。

### 待真机复验（v22 重点）
1. **⭐ FrostOpacity = 0** → 应为**纯模糊壁纸**（不再白！这是 v22 核心验证点）
2. **FrostOpacity = 80~120** → 清晰毛玻璃效果（模糊+适度暗色染色）
3. **FrostOpacity = 200** → 重暗染色（应仍可见模糊底纹，不像 v21 图2 那样纯暗）
4. **⭐ Frosted 开 + 换壁纸** → 模糊层刷新（v20 消息专用窗口 + v22 正确捕获 一并验证）
5. 四个盒子应**一致**（不再出现图3 的部分白部分正确）
